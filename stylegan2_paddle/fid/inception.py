...
class Fid_inceptionE_2(nn.Layer):
    ...
    def __init__(self, in_channels, name=None):
        ...
        self.branch3x3dbl_3b = ConvBNLayer(384,
                                           ...
                                           name=name + '.branch3x3dbl_3b')
        ### same with paper
        self.branch_pool0 = nn.MaxPool2D(kernel_size=3, stride=1, padding=1)
        self.branch_pool = ConvBNLayer(in_channels,
                                       192,
                                       1,
                                       name=name + '.branch_pool')
...
class ConvBNLayer(nn.Layer):
    def __init__(self,
        ...
        self.conv = nn.Conv2D(in_channels,
                           ...
                           padding=padding,
                           groups=groups,
                           weight_attr=paddle.fluid.ParamAttr(name=name + ".conv.weight"),
                           bias_attr=False)
        self.bn = nn.BatchNorm2D(num_filters,
                            epsilon=0.001,
                            weight_attr=paddle.fluid.ParamAttr(name=name + ".bn.weight"),
                            bias_attr=paddle.fluid.ParamAttr(name=name + ".bn.bias"))
        self.bn._mean.name = name + '.bn.running_mean'
        self.bn._variance.name = name + '.bn.running_var'
